...
def plot_weight_heatmap(weights):
    plt.figure(figsize=(10, 5))
    plt.imshow(weights.detach().numpy(), cmap='hot', interpolation='nearest')
    plt.colorbar()
...
@add_start_docstrings(
    """Bert for frequency rank in protein PTK7 """ ,
    BERT_START_DOCSTRING ,
)
class BertForFrequency(BertPreTrainedModel):
    def __init__(self , config):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class BertEncoderForDecoder(nn.Module):
    def __init__(self , config,attention_hiddensize=768,attention_layers=6):
        super().__init__()
        config.hidden_size=attention_hiddensize
        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.layer = nn.ModuleList([BertLayer(config) for _ in range(attention_layers)])
...
class Decoder(BertPreTrainedModel):
    ...
    The model can behave as an encoder (with only self-attention) as well
    as a maskdecoder, in which case a layer of cross-attention is added between
    the self-attention layers, following the architecture described in `Attention is all you need`_ by Ashish Vaswani,
    Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N. Gomez, Lukasz Kaiser and Illia Polosukhin.

    To behave as an maskdecoder the model needs to be initialized with the
    :obj:`is_decoder` argument of the configuration set to :obj:`True`; an
    :obj:`encoder_hidden_states` is expected as an input to the forward pass.
    ...
    def __init__(self , config):
        ...
        self.encoder = BertEncoderForDecoder(config)
        self.pooler = BertPooler(config)
    ...
    Examples::
        ...
        if hidden_states is not None and inputs_embeds is not None:
            raise ValueError("You cannot specify both input_ids and inputs_embeds at the same time")
        ...
        #        elif attention_mask.dim() == 2:
        #            # Provided a padding mask of dimensions [batch_size, seq_length]
        #            # - if the model is a maskdecoder, apply a causal mask in addition to the padding mask
        #            # - if the model is an encoder, make the mask broadcastable to [batch_size, num_heads, seq_length, seq_length]
        #            if self.config.is_decoder:
        #                batch_size, seq_length = input_shape
        #                seq_ids = torch.arange(seq_length, device=device)
...
@add_start_docstrings("""Bert Model with a `language modeling` head on top. """, BERT_START_DOCSTRING)
class BertForMaskedLM_ori(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
@add_start_docstrings("""Bert Model with a `language modeling` head on top. """ , BERT_START_DOCSTRING)
class BertForMaskedLM(BertPreTrainedModel):
    def __init__(self , config , down_dim=8):
        super().__init__(config)

        self.bert = BertModel(config)

        self.cls = BertOnlyMLMHead(config)
        self.down = nn.Linear(config.vocab_size , down_dim)
        self.act = nn.ReLU()
        self.up = nn.Linear(down_dim , config.vocab_size)
    ...
    Returns:
        ...
        sequence_output = outputs[0]
        h1 = self.cls(sequence_output)

        h2 = self.down(h1)
        h3 = self.act(h2)
        prediction_scores = self.up(h3)

        outputs = (prediction_scores , h2) + outputs[
        ...
        #    of predictions for masked words.
        # 2. If `lm_labels` is provided we are in a causal scenario where we
        #    try to predict the next token for each input in the maskdecoder.
        if masked_lm_labels is not None:
            loss_fct = CrossEntropyLoss()  # -100 index = padding token
            masked_lm_loss = loss_fct(prediction_scores.view(-1 , self.config.vocab_size) , masked_lm_labels.view(-1))
            outputs = (masked_lm_loss ,) + outputs
...
class BertForCLS(BertPreTrainedModel):
    def __init__(self , config , down_dim=8,label_num=2):
        super().__init__(config)
        self.label_nums=label_num
        self.bert = BertModel(config)
        ...
        self.head = nn.Sequential(
           nn.Linear(down_dim, 256),
           nn.Linear(256, 256),
            nn.Linear(256, self.label_nums)
        )
        self.init_weights()
    ...
    Returns:
        ...
        prediction_scores=self.head(h3.squeeze())
        outputs = (prediction_scores , h2) + outputs[
                                             2:]  # Add hidden states and attention if they are here
        ...
        if masked_lm_labels is not None:
            ...
            masked_lm_loss = loss_fct(prediction_scores.view(-1 , self.label_nums) , masked_lm_labels.view(-1))
            outputs = (masked_lm_loss ,) + outputs
...
@add_start_docstrings("""Bert Model with a `language modeling` head on top. """ , BERT_START_DOCSTRING)
class BertForReg(BertPreTrainedModel):
    def __init__(self , config , down_dim=8,embedding_len=29,label_num=1):
        super().__init__(config)
        self.label_nums=label_num
        self.bert = BertModel(config)
        ...
        class Reshape(nn.Module):
            def __init__(self , *args):
                ...
                self.shape = args
            def forward(self , x):
                return x.transpose(self.shape[0],self.shape[1])



        self.dense=nn.Sequential(
            nn.ReLU() ,
            nn.Linear(down_dim,config.hidden_size),
            nn.ReLU(),
            nn.Linear(config.hidden_size,config.hidden_size),
            nn.ReLU(),
            nn.Linear(config.hidden_size , config.hidden_size) ,
            nn.ReLU() ,
            nn.Linear(config.hidden_size,down_dim),
            nn.Linear(down_dim , 1),
        )
        self.head = nn.Sequential(
            nn.Linear(embedding_len , self.label_nums)
        )
        self.init_weights()
    ...
    Returns:
        ...
        #    try to predict the next token for each input in the maskdecoder.
        if masked_lm_labels is not None:
            loss_fct = MSELoss()  # -100 index = padding token
            masked_lm_loss = loss_fct(prediction_scores.view(-1 , self.label_nums) , masked_lm_labels.view(-1))
            outputs = (masked_lm_loss ,) + outputs
...
@add_start_docstrings("""Bert Model with a `language modeling` head on top. """ , BERT_START_DOCSTRING)
class BertForGenerate(BertPreTrainedModel):
    def __init__(self , config , down_dim=8,use_CLS_gen=False,):
        super().__init__(config)
        self.use_CLS_gen=use_CLS_gen
        self.bert = BertModel(config)
        ...
        self.down = nn.Linear(config.vocab_size , down_dim)
        self.act = nn.ReLU()
        self.up_decoder = nn.Linear(down_dim , config.hidden_size)
        if self.use_CLS_gen:
            self.cls_token_mlp_decoder=nn.Linear(1,config.seq_len)
        self.decoder = Decoder(config)
        self.cls_decoder = BertOnlyMLMHead(config)
        self.init_weights()
    ...
    Returns:
        ...
        # 2. If `lm_labels` is provided we are in a causal scenario where we
        #    try to predict the next token for each input in the maskdecoder.
        h2 = self.up_decoder(hidden_outputs)
        if self.use_CLS_gen:
            h2=h2.unsqueeze(2)
            h3=self.cls_token_mlp_decoder(h2)
            h2=h3.permute(0,2,1)
        ...
        prediction_scores = self.cls_decoder(sequence_output)

        outputs = (prediction_scores , sequence_output , hidden_outputs)
...
@add_start_docstrings("""Bert Model with a `language modeling` head on top. """ , BERT_START_DOCSTRING)
class BertForMLPGenerate(BertPreTrainedModel):
    def __init__(self , config , down_dim=8):
        super().__init__(config)
        self.bert = BertModel(config)
        ...
        self.down = nn.Linear(config.vocab_size , down_dim)
        self.act = nn.ReLU()
        self.up_decoder = nn.Linear(down_dim , 2)
        self.decoder_token_mlp = nn.Linear(2,config.vocab_size)
        self.decoder_cls_mlp1=nn.Linear(config.hidden_size, config.seq_len)
        self.decoder_cls_mlp2 = nn.Linear(1, config.vocab_size)
        self.init_weights()
    ...
    Returns:
        ...
        sequence_output = h2
        if use_CLS_gen:
            h3=self.decoder_cls_mlp1(sequence_output)
            h4=h3.unsqueeze(2)
            prediction_scores=self.decoder_cls_mlp2(h4)
        else:
            prediction_scores = self.decoder_token_mlp(sequence_output)

        outputs = (prediction_scores , sequence_output , hidden_outputs)
        if plot_weight:
            plot_weight_distribution(self.decoder_token_mlp.weight.cpu().detach().numpy())
        if masked_lm_labels is not None:
            loss_fct = CrossEntropyLoss()  # -100 index = padding token
            masked_lm_loss = loss_fct(prediction_scores.view(-1 , self.config.vocab_size) ,
                                      masked_lm_labels.contiguous().view(-1))
